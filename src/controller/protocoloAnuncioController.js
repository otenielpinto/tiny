import { ProtocoloAnuncioRepository, TProtocolo } from "../repository/protocoloAnuncioRepository.js";
...
const create = async (req, res) => {
  ...
  let response = await tiny.post("produto.incluir.php", data);
...
const get = async (req, res) => {
  TResponseService.send(req, res, await TProtocolo.obterAnuncio(req.params.codigo));
}
...
async function updateVariacoes(produto, variacoes) {
  const sys_variacoes = produto?.variacoes
  if (!variacoes || !Array.isArray(sys_variacoes)) return variacoes
  for (let v of variacoes) {
    let item = sys_variacoes.find(sys => String(sys?.variacao?.codigo) === String(v?.id_produto))
    if (item) {
      v.id_variant_mktplace = item?.variacao?.id
      v.id_anuncio_mktplace = produto?.id;
    }
  }
  return variacoes
}
